...
int main(int, char**)
{
  ...
  ink.camera.set_fov(30.0);
  ink.camera.set_transform(look_at(Point3f(50, 52, 295.6), Point3f(50, 52, 295.6) + Point3f(0, -0.042612, -1), Vec3f(0, 1, 0)));

  // Setup scene (see smallpt)
  Sphere sphere_wall(1e5);
  ink.add_instance(&sphere_wall, translate(1e5 + 1, 40.8, 81.6), Vec3f(.25, .25, .75));		// right
  ink.add_instance(&sphere_wall, translate(-1e5 + 99, 40.8, 81.6), Vec3f(.75, .25, .25)); // left
  ...
  Sphere sphere(16.5);
  ink.add_instance(&sphere, translate(73, 16.5, 47), Vec3f(0.0, 0.0, 0.0)); // mirror
